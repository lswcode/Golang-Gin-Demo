...
func InitRouter() *gin.Engine {
	...
	router.LoadHTMLGlob("views/*") // 设置ctx.HTML返回html文件时的路径前缀，这里只写了一个/，所以views下面不能再有文件夹
	...
	// 需要加小括号调用return的函数
	router.GET("/mw", middleware.M2(), middleware.M3(), controllers.UserController) // 单个路由的局部中间件

	// 中间件执行顺序: 全局中间件 —> 路由组中间件 —> 路由中间件
